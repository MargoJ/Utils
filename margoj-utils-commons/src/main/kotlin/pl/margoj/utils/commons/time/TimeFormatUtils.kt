package pl.margoj.utils.commons.time
...
object TimeFormatUtils
{
    ...
    /**
     * Zamiania czas w milisekundach na skrótową forme zapisu
     */
    fun toParsableTime(time: Long): String
    {
        var currentTime = time / 1000L
        val out = StringBuilder()

        fun processTimePart(convert: (Long) -> Long, format: TimeUnit, char: Char)
        {
            val units = convert(currentTime)
            if (units == 0L)
            {
                return
            }

            currentTime -= format.toSeconds(units)
            out.append(units).append(char)
        }

        processTimePart(TimeUnit.SECONDS::toDays, TimeUnit.DAYS, 'd')
        processTimePart(TimeUnit.SECONDS::toHours, TimeUnit.HOURS, 'h')
        processTimePart(TimeUnit.SECONDS::toMinutes, TimeUnit.MINUTES, 'm')
        processTimePart(TimeUnit.SECONDS::toSeconds, TimeUnit.SECONDS, 's')
